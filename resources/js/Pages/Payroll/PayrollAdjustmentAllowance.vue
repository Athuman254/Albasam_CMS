<template>
   <div class="col-xl-12">
      <div class="card">
         <div class="card-header flex-column flex-md-row">
            <div class="row row-gap-1">
               <div class="col-md-3 col-9">
                  ...
                  v-model="appendParams.filter.search"
                     @input="applyFilter">
               </div>
         ...
         <VueTable :fields="fields" api-url="datatable/allowance/adjustments" :append-params="appendParams"
            ref="allowanceTable">
            ...
            <template #actions="props">
               <div class="dropdown">
                  ...
                  <div class="dropdown-menu dropdown-menu-end">
                     <a v-if="!props.rowData.is_included" class="dropdown-item" href="#" @click="openUpdateModal(props.rowData)">
               ...
               <form id="createForm" @submit.prevent="createEmployeeAllowance">
                  <div class="modal-body">
                     ...
                     <div class="mb-3">
                        ...
                        <v-select v-model="form.allowance_id" :options="allowances" label="name"
                           :reduce="option => option.id"></v-select>
 ...
 <!-- Edit Modal -->
      <div class="modal fade" id="create-rank-modal" data-bs-backdrop="static" tabindex="-1"
         aria-labelledby="create-rank-modal-label" aria-hidden="true" ref="editModal">
         <div class="modal-dialog">
            <div class="modal-content">
               <div class="modal-header">
                  <h5 class="modal-title" id="create-rank-modal-label">Edit Allowance</h5>
                  <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"
                     @click="formCleanUp"></button>
               </div>
                  ...
                  <div class="modal-body">
                     <div class="mb-3">
                        ...
                        <v-select v-model="editForm.employee_id" :options="employees" label="first_name"
                           :reduce="option => option.id"></v-select>
                        <div v-if="editForm.errors.employee_id" class="text-danger">{{ editForm.errors.employee_id }}</div>
                     </div>

                     <div class="mb-3">
                        <label for="divisionId" class="form-label">Allowance</label>
                        <v-select v-model="editForm.allowance_id" :options="allowances" label="name"
                           :reduce="option => option.id"></v-select>
                        <div v-if="editForm.errors.allowance_id" class="text-danger">{{ editForm.errors.allowance_id }}</div>
                     </div>
...
const openUpdateModal = (rowData) => {
   ...
   editForm.allowance_id = rowData.allowance_id
...
const applyFilter = _debounce(function () {
         allowanceTable.value.reloadTable();
      }, 800)
...
const createEmployeeAllowance = () => {
   form.post(route('admin.allowance.adjustment'), {
      onSuccess: () => {
          allowanceTable.value.reloadTable();
...
const updateEmployeeAllowance = () =>{
   editForm.patch(route('admin.allowance.adjustment.update', editForm.id),{
      onSuccess: () => {
         allowanceTable.value.reloadTable();
         closeUpdateModal()
      },
