<template>
  <Head :title="`Assign Classes & Subjects - ${employee.full_name}`" />
  <DefaultLayout>
    <div class="row">
      <div class="col-12">
        <div class="card">
          <div class="card-header d-flex justify-content-between align-items-center">
            <div>
              <h4 class="card-title mb-1">Assign Classes & Subjects to {{ employee.full_name }}</h4>
            ...
            <div>
              <Link :href="route('admin.employees.show', employee.id)" class="btn btn-outline-secondary btn-sm">
                <i class="bx bx-arrow-back me-1"></i> Back to Employee
              </Link>
            </div>
          </div>
          <div class="card-body">
                ...
                <div class="col-md-3">
                  <div class="card bg-primary text-white">
                    <div class="card-body text-center p-2">
                      ...
                      <h4 class="mb-0">{{ statistics.totalClasses }}</h4>
                    </div>
                  </div>
                </div>
                <div class="col-md-3">
                  <div class="card bg-info text-white">
                    <div class="card-body text-center p-2">
                      ...
                      <h4 class="mb-0">{{ statistics.totalSubjects }}</h4>
                    </div>
                  </div>
                </div>
                <div class="col-md-3">
                  <div class="card bg-success text-white">
                    <div class="card-body text-center p-2">
                      <h6 class="card-title mb-0">Class Teacher</h6>
                      <h4 class="mb-0">{{ statistics.classTeacherCount }}</h4>
                    </div>
                  </div>
                </div>
                <div class="col-md-3">
                  <div class="card bg-warning text-white">
                    <div class="card-body text-center p-2">
                      <h6 class="card-title mb-0">Subject Teacher</h6>
                      <h4 class="mb-0">{{ statistics.subjectTeacherCount }}</h4>
                    </div>
                  </div>
                </div>
              </div>
        ...
        <div class="modal-content">
          ...
          <div class="modal-body">
            ...
            <div v-if="assignmentForm.class_id" class="alert" :class="getPreviewAlertClass()">
              ...
              <div v-if="hasExistingClassTeacher" class="mt-2 text-warning">
                <i class="bx bx-info-circle"></i> There is already a class teacher for this class. 
                This will replace the existing class teacher.
              </div>
              <div v-if="currentSubjectCount > 0 && !assignmentForm.is_class_teacher" class="mt-2 text-info">
                <i class="bx bx-info-circle"></i> This teacher currently teaches {{ currentSubjectCount }} subject(s) in this class.
                You can add up to {{ 2 - currentSubjectCount }} more subject(s).
              </div>
            </div>
...
const statistics = computed(() => {
  const totalClasses = [...new Set(assignments.value.map(a => a.class_id))].length
  const subjectIds = assignments.value.map(a => a.subject_id).filter(id => id)
  const totalSubjects = [...new Set(subjectIds)].length
  
  return {
    totalClasses: totalClasses,
    totalSubjects: totalSubjects,
    classTeacherCount: assignments.value.filter(a => a.is_class_teacher).length,
    subjectTeacherCount: assignments.value.filter(a => !a.is_class_teacher && a.subject_id).length,
    totalAssignments: assignments.value.length
  }
})

const hasExistingClassTeacher = computed(() => {
  if (!assignmentForm.class_id || !academicYearId.value) return false
  
  return assignments.value.some(assignment => 
    assignment.class_id == assignmentForm.class_id && 
    assignment.academic_year_id == academicYearId.value &&
    assignment.is_class_teacher &&
    (!editingAssignment.value || !editingAssignment.value.assignment_ids?.includes(assignment.id))
  )
})

const currentSubjectCount = computed(() => {
  if (!assignmentForm.class_id || !academicYearId.value) return 0
  
  const classAssignments = assignments.value.filter(assignment => 
    assignment.class_id == assignmentForm.class_id && 
    assignment.academic_year_id == academicYearId.value &&
    !assignment.is_class_teacher
  )
  
  return classAssignments.length
})
...
const editClassAssignment = (classAssignment) => {
  ...
  assignmentForm.is_class_teacher = false // Subject teachers can't become class teachers via edit
...
const submitAssignment = async () => {
  ...
  if (!assignmentForm.is_class_teacher && assignmentForm.subject_ids.length > 2) {
    assignmentForm.errors.subject_ids = 'Maximum 2 subjects allowed per class'
    return
  }
  ...
  try {
    const url = editingAssignment.value 
      ? route('admin.employees.class-assignments.update', { 
          employee: props.employee.id, 
          assignment: 'bulk' // Use bulk update for multiple subjects
        })
      : route('admin.employees.class-assignments.store', props.employee.id)

    const method = editingAssignment.value ? 'PUT' : 'POST'

    console.log('Submitting assignment to:', url, 'with method:', method)
    console.log('Data:', {
      class_id: assignmentForm.class_id,
      subject_ids: assignmentForm.subject_ids,
      is_class_teacher: assignmentForm.is_class_teacher,
      academic_year_id: academicYearId.value,
      // For editing, include the assignment IDs to update
      assignment_ids: editingAssignment.value ? editingAssignment.value.assignment_ids : null
    })
...
const removeClassAssignment = async (classAssignment) => {
  ...
  try {
    // For bulk deletion, we need to send all assignment IDs
    const url = route('admin.employees.class-assignments.remove', { 
      employee: props.employee.id
    })
    
    console.log('Deleting assignments from:', url)
    console.log('Assignment IDs to delete:', classAssignment.assignment_ids)
...
// Debug routes on component mount
console.log('Available routes for debugging:')
console.log('Store route:', route('admin.employees.class-assignments.store', props.employee.id))
console.log('Update route:', route('admin.employees.class-assignments.update', { 
  employee: props.employee.id, 
  assignment: 'test-id' 
}))
console.log('Remove route:', route('admin.employees.class-assignments.remove', { 
  employee: props.employee.id
}))
console.log('Get assignments route:', route('admin.employees.get-employee-assignments', {
  employee: props.employee.id,
