              ...
              <div class="modal-dialog">
                    <div v-if="selectedRecord !== null" class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="create-rank-modal-label">{{ selectedRecord?.student.first_name }} {{ selectedRecord?.student.last_name }}</h5>
                            <button
                                type="button"
                                class="btn-close"
                        ...
                        <div class="modal-body">
                                <div class="mb-2"><span class="text-muted fw-bold">Adm no: </span>  {{ selectedRecord?.student.student_admission_id }}</div>
                                <div class="mb-2"><span class="text-muted fw-bold">Class: </span>{{ selectedRecord?.rank.name }}</div>
                                <span :class="'p-2 rounded  '+getStatusColor(selectedRecord?.status)">{{ selectedRecord?.status }} On {{ selectedRecord?.date }}</span>
                                <div class="form my-3">
                                    <label class="mb-2">Remarks</label>
                                    <textarea disabled class="form-control"  name="" id="">{{ selectedRecord?.remarks }}</textarea>
                                </div>
                                <span class="text-muted fw-bold">Done by:</span>  {{ selectedRecord?.teacher.employee.first_name }} - {{ selectedRecord?.teacher.employee.last_name }}
                        </div>
