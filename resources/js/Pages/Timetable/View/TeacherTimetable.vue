...
const onTeacherChange = (e) => {
    router.get(route('timetable.view.teacher', e.target.value), { academic_year_id: props.currentAcademicYearId });
};

const onAcademicYearChange = (e) => {
    router.get(route('timetable.view.teacher', props.currentTeacherId), { academic_year_id: e.target.value });
};
...
<template>
    <Head title="Teacher Timetable" />

    <DefaultLayout>
        <template #header>
            <div class="flex justify-between items-center print:hidden">
                <h2 class="font-semibold text-xl text-gray-800 leading-tight">Teacher Timetable</h2>
                ...
                <div v-else class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6 print:shadow-none">
                    <div class="text-center mb-6">
                        <h1 class="text-2xl font-bold text-gray-900">
                            {{ teachers.find(t => t.id === currentTeacherId)?.full_name }}'s Timetable
                        </h1>
                                        ...
                                        <td v-for="day in daysOfWeek" :key="day" class="border border-gray-300 p-2 text-center relative hover:bg-gray-50 transition">
                                            <div v-if="getAllocation(day, period.id)" class="h-full w-full">
                                                ...
                                                <div class="text-xs text-gray-600 mt-1">
                                                    {{ getAllocation(day, period.id).class.name }}
                                                </div>
